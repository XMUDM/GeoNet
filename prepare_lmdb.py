...
import argparse
from lightnp.LSRM.utils.build_group_graph import *
...
class MD22(DownloadableAtomsData):
    ...
    datasets_dict = dict(
        ...
        buckyball_catcher = "md22_buckyball_catcher.npz",
...
def parse_args(jupyter = False):
    ...
    parser.add_argument('--dataset', type = str, default = 'MD', choices= ['MD17', 'MD22'] , help = 'Dataset to process')
        ...
        try:
            ...
            if label_builder != 'rdkit':
                ...
                if label_builder == "spectral_two":
                    ...
                    build_grouping_graph(data)    
                    build_neighborhood_n_interaction(data)# eighborhood(data)
            else:
...
def main(mol,config):      
    ...
    energy_mean = torch.mean(torch.tensor(energys)).item()
    energy_std = torch.std(torch.tensor(energys)).item()
