...
def parse_args(jupyter=False):
    ...
    # 构象一致性损失相关选项
    parser.add_argument('--use_conformer_loss', action='store_true', default=True, help='是否使用构象一致性损失')
    parser.add_argument('--conformer_loss_weight', type=float, default=0.0001, help='构象一致性损失的权重')
    parser.add_argument('--brics_consistency_strength', type=float, default=0.5, help='BRICS片段内部特征一致性的目标强度(0-1之间)')
    parser.add_argument('--vector_weight', type=float, default=0, help='向量特征一致性约束强度')
    parser.add_argument('--short_long_ratio', type=float, default=0.4, help='短程和长程特征的权重比例')
    parser.add_argument('--atom_identity_weight', type=float, default=0.5, help='原子身份一致性约束强度')
    # parser.add_argument('--conformer_clusters', type=int, default=5, help='构象特征聚类的数量，用于隐式分组')
...
def main(world_size, config):
    ...
    total_params = cal_num_trainable_parm(model)
    if config['local_rank'] == 0:
        print(f"\n{'='*50}")
    ...
    # 获取构象一致性损失权重，默认为0.0（不使用）
    conformer_loss_weight = config.get("conformer_loss_weight", 0.1) if config.get("use_conformer_loss", False) else 0.0
    print(f"conformer_loss_weight: {conformer_loss_weight}")
    # 构建带有构象一致性损失的损失函数
    loss_fn = ltnp.train.loss.build_mse_loss_with_forces(
        rho_tradeoff=config["rho_tradeoff"],
        with_forces=config["regress_forces"],
        conformer_loss_weight=conformer_loss_weight
    )
...
if __name__ == '__main__':
    ...
    print(config['num_interactions'])
    print('rho_tradeoff')
    print(config['rho_tradeoff'])
