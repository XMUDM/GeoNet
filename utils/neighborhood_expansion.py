...
def neighborhood_expansion(g, labels):
    ...
    edge_index_labeled_group = labels[g.edge_index] # Get the labels of (s,t) -> (s_label, t_label) 形状为[2, num_edges]，表示每条边两端节点的组标签
    num_nodes = g.num_nodes
    num_labels = g.num_labels
    filters = edge_index_labeled_group[0] != edge_index_labeled_group[1] # s_label != t_label means connections are in different groups [True, True, False, True, False]（标识跨组边）
    edges_crossing_group = g.edge_index[:, filters]#只保留跨组边
