...
class DDPTrainer:
    ...
    def forward_batch(self, batch_data):
        ...
        return result, loss, contrastive_loss
        ...
        with self.ema.average_parameters():
            ...
            for eval_batch in loader:
                ...
                if not self.train_force:
                    with torch.no_grad():
                        result, loss, contrastive_loss = self.forward_batch(eval_batch)
                        eval_loss += loss.detach().cpu().numpy()
                        eval_contrastive_loss += contrastive_loss
                else:
                    result, loss, contrastive_loss = self.forward_batch(eval_batch)
                    eval_loss += loss.detach().cpu().numpy()
                    eval_contrastive_loss += contrastive_loss
                ...
                if self.local_rank == 0 and not self.config['amlt']:
                    ...
                    loop.set_postfix(loss=loss.detach().cpu().numpy(), contrastive=contrastive_loss)
                    loop.update(1)
            
            if self.world_size>1:
                # reduce loss from each childern.
                eval_loss = torch.Tensor([eval_loss]).to(self.device)
                eval_contrastive_loss = torch.Tensor([eval_contrastive_loss]).to(self.device)
                dist.all_reduce(eval_loss, op=dist.ReduceOp.SUM)
                dist.all_reduce(eval_contrastive_loss, op=dist.ReduceOp.SUM)
                eval_loss = eval_loss.cpu().numpy().item()
                eval_contrastive_loss = eval_contrastive_loss.cpu().numpy().item()
                eval_loss = eval_loss / self.world_size / len(loader)
                eval_contrastive_loss = eval_contrastive_loss / self.world_size / len(loader)
                
                dist.barrier()
                
            if self.local_rank == 0:
                tqdm.write("epoch : {} , {} loss average is : {:.6f}, contrastive loss average is : {:.6f}".format(
                    self.epoch, loader_name, eval_loss, eval_contrastive_loss))
            return eval_loss, eval_contrastive_loss
    ...
    def train_epoch(self, loader):
        ...
        for train_batch in loader:
            ...
            if self.fp16:
                with torch.cuda.amp.autocast():
                    result, loss, contrastive_loss = self.forward_batch(train_batch)
            else:
                result, loss, contrastive_loss = self.forward_batch(train_batch) 
        ...
        train_contrastive_loss = torch.Tensor([train_contrastive_loss]).to(self.device)
        dist.all_reduce(train_loss, op=dist.ReduceOp.SUM)
        dist.all_reduce(train_contrastive_loss, op=dist.ReduceOp.SUM)
        train_loss = train_loss.cpu().numpy().item()
        train_contrastive_loss = train_contrastive_loss.cpu().numpy().item()
        train_loss = train_loss / self.world_size / len(loader)
        train_contrastive_loss = train_contrastive_loss / self.world_size / len(loader)
        
        dist.barrier()
        if self.local_rank == 0:
            tqdm.write("epoch : {} train loss average is : {:.6f}, contrastive loss average is : {:.6f}".format(
                self.epoch, train_loss/len(loader), train_contrastive_loss/len(loader)))
        

        return train_loss, train_contrastive_loss
    ...
    def train(self, n_epochs = 100000,loader = None):
        ...
        try:
            for _ in range(n_epochs):
                ...
                #---------------train--------------------
                train_loss, train_contrastive_loss = self.train_epoch(self.train_loader)
                log_name.extend(["train/train_loss", "train/contrastive_loss"])
                log_val.extend([train_loss, train_contrastive_loss])
                self.update_metrics2log(log_name,log_val,name_prefix = "train",clear_metrics=True)
                
                #---------------val--------------------
                val_loss, val_contrastive_loss = self.evaluate_epoch(self.validation_loader,loader_name="validation_loader")
                log_name.extend(["val/val_loss", "val/contrastive_loss"])
                log_val.extend([val_loss, val_contrastive_loss])
                ...
                if self.epoch == 10 or self._stop or (self.epoch % self.test_interval == 0 and self.test_loader is not None):
                    ...
                    torch.cuda.empty_cache()
                    test_loss, test_contrastive_loss = self.evaluate_epoch(self.test_loader,loader_name="test_loader")
                    torch.cuda.empty_cache()
                    log_name.extend(["test/test_loss", "test/contrastive_loss"])
                    log_val.extend([test_loss, test_contrastive_loss])
    ...
    def test(self, loader = None):
        ...
        test_loss, test_contrastive_loss = self.evaluate_epoch(loader,loader_name="test_loader")
        log_name = ["test_loss", "test_contrastive_loss"]
        log_val = [test_loss, test_contrastive_loss]
