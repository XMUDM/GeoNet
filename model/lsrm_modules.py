...
from .long_short_interact_modules import LongShortIneractModel_dis_direct_vector2_drop, ImprovedLongShortInteractModel, LongShortIneractModel_graph_aware
...
class BipartiteEdgeWithAngleFeatures(nn.Module):
    ...
    def __init__(self,
        ...
        self._angle_extractor_initialized = False
        self.mode = ['train', 'valid', 'test']
        if use_angle_features:
            print("[INFO] 启用DimeNet风格角度特征提取，将在第一次forward时根据edge_index动态初始化")

    def _initialize_angle_extractor(self, edge_index):
        """根据edge_index延迟初始化角度特征提取器"""
        if not self._angle_extractor_initialized and self.use_angle_features:
            # 从edge_index获取设备信息，避免模块没有参数时的错误
            ...
            # 将新创建的角度特征提取器移到正确的设备上
            self.angle_extractor = self.angle_extractor.to(device)
            
            self._angle_extractor_initialized = True
            print(f"[INFO] DimeNetStyleAngleFeatureExtractor已初始化并移到设备 {device}，edge_index形状: {edge_index.shape}")

    def forward(self, edge_index, node_pos, group_pos, triplet_i, triplet_j, triplet_k, id_expand_kj=None, id_reduce_ji=None, node_vec=None, group_vec=None, mode="train",*args, **kwargs):
        """
        Args:
            edge_index: [2, num_edges] 边索引
        ...
        """
        # 第一次调用时初始化角度特征提取器
        if self.use_angle_features and not self._angle_extractor_initialized:
            self._initialize_angle_extractor(edge_index)
        ...
        # 处理夹角特征
        if self.use_angle_features and self.angle_extractor is not None:
            # 使用DimeNet风格的角度特征提取器
            # 基于三元组 (node, edge, group)，其中 edge 位置为 node 和 group 的中点
            edge_rbf_features, angle_attr = self.angle_extractor(
                node_pos, group_pos, edge_attr, triplet_i, triplet_j, triplet_k, id_expand_kj, id_reduce_ji
            )  # [num_edges, hidden_channels] - DimeNet风格的SBF特征
            
            return edge_index, edge_weight, edge_rbf_features, edge_vec_normalized, angle_attr
        else:
            # 如果不使用夹角特征，返回None
            return edge_index, edge_weight, edge_rbf_features, edge_vec_normalized, None
...
class MoECompatibleConsistencyLoss(nn.Module):
    ...
    def compute_fragment_consistency(self, features, fragment_ids, valid_mask=None):
        """计算片段内特征的一致性损失 - 增大同一片段内的相似度
        基于compute_interfragment_differentiation的风格，但关注片段内一致性
        
        Args:
            features: 原子特征 [num_atoms, hidden_dim]
        ...
        # 计算每个原子与其所属片段平均特征的相似度
        # features_norm: [num_atoms, hidden_dim]
        # fragment_features_norm: [num_fragments, hidden_dim]
        # 需要将fragment_features_norm映射到每个原子
        fragment_features_for_atoms = fragment_features_norm[fragment_ids]  # [num_atoms, hidden_dim]
        
        # 计算相似度 (值越高越相似，范围[-1, 1])
        similarity = (features_norm * fragment_features_for_atoms).sum(dim=1)  # [num_atoms]
        
        # 计算损失：让同一片段内的相似度接近1（增大相似度）
        # 损失 = 1 - similarity 的平均值（相似度越高，损失越小）
    ...
    def compute_interfragment_differentiation(self, scalar_features, vector_features, fragment_ids):
        ...
        同时使用长程标量和向量特征建模不同BRICS片段之间的作用力
        
        Args:
            scalar_features: 长程标量特征 [num_atoms, hidden_dim]
            vector_features: 长程向量特征的幅度 [num_atoms, 1]
            fragment_ids: BRICS片段ID [num_atoms]
        """
        if fragment_ids is None:
    ...
    def forward(self, scalar_short, scalar_long, vector_short, vector_long, fragment_ids, atom_types=None):
        ...
        # 重要: 只使用长程特征，不进行混合，避免干扰MoE
        scalar_features = self.feature_projector(scalar_long)
        ...
        if self.vector_weight > 0 and vector_long is not None:
            # 简化向量处理: 只计算向量大小，忽略方向
            vector_magnitudes = torch.norm(vector_long, dim=2).mean(dim=1)  # [batch_size]
...
class Visnorm_shared_LSRMNorm2_2branchSerial(nn.Module):
    def __init__(self,regress_forces = True,
        ...
        for i in range(self.long_num_layers):
            ...
            #                                                                                  cutoff=self.long_cutoff_upper,norm=True, max_group_num = 3,act = "silu",num_heads=8,
            #                                                                                  p = config["dropout"]))
            self.longshortinteract_models.append(LongShortIneractModel_graph_aware(hidden_channels, num_gaussians=50, 
                                                                                             cutoff=self.long_cutoff_upper,norm=True, max_group_num = 3,act = "silu",num_heads=8,
                                                                                             p = config["dropout"]))
        ...
        self.out_energy = OutputNet(hidden_channels*3, act = 'silu', dipole = False, mean = mean, std = std, atomref = atom_ref, scale = None)
    ...
    def forward(self,
        ...
        if self.use_angle_features:
            ...
            # 重要：使用过滤后的data.interaction_graph来计算夹角特征，并传递预计算的DimeNet索引
            edge_index_bipartite, edge_weight_bipartite, edge_attr_bipartite, edge_vec_bipartite, angle_attr_bipartite = self.bipartite_edge_fea_init(
                data.interaction_graph, pos, group_pos, data.id3dnb_i, data.id3dnb_j, data.id3dnb_k, 
                data.id_expand_kj, data.id_reduce_ji, node_vec, initial_group_vec
            )
        else:
            # 原有的距离特征初始化
        ...
        for idx  in range(self.long_num_layers):        
            ...
            if self.use_angle_features and angle_attr_bipartite is not None:
                # 传递分离的边特征和夹角特征
                delta_node_embedding_long, delta_node_vec_long, contrastive_loss = self.longshortinteract_models[idx](
                    ...
                    return_contrastive_loss = True  # 明确要求返回对比损失
                )
                layer_contrastive_loss = contrastive_loss if contrastive_loss is not None else 0.0
            else:
                # 原有调用方式
                delta_node_embedding_long, delta_node_vec_long, contrastive_loss = self.longshortinteract_models[idx](
                    edge_index = edge_index_bipartite, 
            ...
            # 累积对比损失
            if isinstance(layer_contrastive_loss, torch.Tensor):
                total_contrastive_loss += layer_contrastive_loss
                contrastive_loss_count += 1
            elif isinstance(layer_contrastive_loss, (int, float)) and layer_contrastive_loss > 0:
                total_contrastive_loss += layer_contrastive_loss
                contrastive_loss_count += 1
